...
<!--/.navbar-collapse -->
	...
	$sql = "SELECT KorisnikID FROM korisnik WHERE Username='$username'";
	$result = mysqli_query($conn, $sql);
	if (mysqli_num_rows($result) > 0) {
		    while($row = mysqli_fetch_assoc($result)) {
		echo "
	<a href='karte.php?id=" . $row['KorisnikID'] . "'><input type='button' id='register_btn' class='btn btn-primary' value='Moje karte'></a>
	";
	}}
