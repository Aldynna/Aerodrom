...
if(isset($_POST['btn-signup']))
{
	...
	if($username!=null)
	{
		$query= "select * from PutnaKlasa WHERE NazivPutneKlase='$username'";
		$query_run = mysqli_query($conn,$query) or die("I cannot connect to the database because: " . mysqli_error($conn));;
		if(mysqli_num_rows($query_run)>0)
		{
				echo '<script type="text/javascript"> alert("Klasa je vec u bazi, unesite drugu...") </script>';
		} 
		
		else
		{
			
			$query= "insert into PutnaKlasa (NazivPutneKlase) values('$username')";
			$query_run = mysqli_query($conn,$query);
			if($query_run)
			{	
				echo '<script type="text/javascript"> alert("Putna klasa uspjesno dodana..") </script>';
	...
	}else
	{
			...
			header('location:addklasa.php?gr=n');
	}
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
            <form role="form" method="post"  >
                <h2>Unesite novu putnu klasu</h2>
                <hr class="colorgraph">
