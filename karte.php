...
<!--/.navbar-collapse -->
	...
	<a href='karte.php?username=" . $row['username'] . "'><input type='button' id='register_btn' class='btn btn-primary' value='Kupi kartu'></a>
		<table id="myTable" style="width:80%;" class="linija">
			<form action='drugiadmin.php' method='post'>
				<tr>
					<th style="height: 50px;  text-align: center; background-color: #699bcc; color: white;">KartaID</th>
					<th style="height: 50px;  text-align: center; background-color: #699bcc; color: white;">RedLetenjaID</th>
					<th style="height: 50px;  text-align: center; background-color: #699bcc; color: white;">KorisnikID</th>
					<th style="height: 50px;  text-align: center; background-color: #699bcc; color: white;">Cijena_CijenaPopustID</th>
					<th style="height: 50px;  text-align: center; background-color: #699bcc; color: white;">Datum</th>
					<th style="height: 50px;  text-align: center; background-color: #699bcc; color: white;">Password</th>
					<th style="height: 50px;  text-align: center; background-color: #699bcc; color: white;">Obriši</th>
				</tr>
			
<?php 
$id = $_GET['id'];
$sql = "SELECT * FROM Karta WHERE KorisnikID='$id'";
...
$counter++;
	echo 
			"<tr >
				<td id='uname'>". $row["KartaID"]. "</td> 
				<td> " . $row["RedLetenjaID"]. " </td>
				<td> " . $row["KorisnikID"]. " </td>
				<td> " . $row["Cijena_CijenaPopustID"]. " </td>
             <td> " . $row["Datum"]. " </td>
				...
				<td><a href='delete.php?username=" . $row['KartaID'] . "'><input type='button' id='register_btn' class='btn btn-primary' value='Obriši'></a></td> 
				...
				<td  class='linija'>
					<div class='collapse' id='$counter' aria-expanded='false'>
						<form  method='post' class='navbar-form navbar-right'  name='poc' role='form' > Uredi korisnika: <br>
							...
							<a href='edit.php?username=" . $row['KartaID'] . "&counter=".$counter."'><input type='button' id='register_btn' class='btn btn-primary' value='Snimi' /></a>
